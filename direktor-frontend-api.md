...
> **Scope**: Toshkent viloyati barcha 14 tumani (read + approve)
> ❌ `/api/users/*` endpointlari direktor uchun **403** qaytaradi.
...
Direktor barcha stats endpointlarga kirish huquqiga ega (`admin | direktor | moderator`). Natijalar server tomonida 60 soniya keshlanadi.
...
| `from` | string? | — | Boshlanish sanasi `YYYY-MM-DD` |
| `to` | string? | — | Tugash sanasi `YYYY-MM-DD` |
| `district_ids` | string? | — | Vergul bilan ajratilgan tuman nomlari. Bo'sh = barcha 14 ta |
...
```json
[
  { "period": "2026-01-01T00:00:00+00:00", "value": 12 },
  { "period": "2026-02-01T00:00:00+00:00", "value": 18 },
  { "period": "2026-03-01T00:00:00+00:00", "value": 25 }
]
```
...
| Param | Turi | Default | Tavsif |
|-------|------|---------|--------|
| `override` | bool | `false` | Tuman farqini chetlab o'tish (`admin`/`direktor` uchun) |

**Biznes qoidalari:**
- Yosh va masulning `district_id` si bir xil bo'lishi kerak
- Tuman farq qilsa va `?override=true` bo'lsa — faqat `admin`/`direktor` ruxsat beradi
- `?override=true` bilan biriktirish `audit_log` ga `action = "youth.assign.override"` yozadi
...
## 3. Removal Workflow (Ro'yxatdan chiqarish)
...
## 7. Tashkilotlar (Organizations) — Yozish
...
CSV formatda eksport. Direktor butun agentlik bo'ylab yoki bitta tuman bo'yicha eksport qila oladi.
...
| `from` | string? | — | Boshlanish sanasi `YYYY-MM-DD` |
| `to` | string? | — | Tugash sanasi `YYYY-MM-DD` |
| `district_id` | string? | — | Faqat bitta tuman |
| `include_pii` | bool | `false` | `true` = to'liq ismlar. PII reveal (`POST /api/pii/reveal`) 5 daqiqa oldin bajarilgan bo'lishi shart |
...
`include_pii=false` bo'lganda `Ism` ustunida faqat initsiallari: `A. T.`

**403** `pii_reveal_required` — `include_pii=true` so'raldi lekin reveal qilinmagan.

---

### `GET /api/reports/agency.pdf` *(v2)*

PDF formatda eksport. Server tomonida render qilinadi.
...
## 9. Taqiqlangan endpointlar

Quyidagi endpointlarga direktor token bilan so'rov yuborsa **403** qaytadi:

| Endpoint | Sabab |
|----------|-------|
| `* /api/users/*` | Foydalanuvchilarni boshqarish faqat `admin` uchun |

Shuningdek, moderator uchun yopiq bo'lgan barcha yozish endpointlari (`POST/PATCH/DELETE /api/flags`, `POST /api/pii/reveal` va h.k.) direktor uchun ham tegishlicha cheklangan.
...
1. **Pagination**: Barcha list endpointlar `{ data: [...], meta: { total, page, limit } }` qaytaradi. TanStack Query `placeholderData: keepPreviousData` bilan ishlating.

2. **District selector (Topbar)**: Direktor `<Topbar />`da 14 ta tuman + "Barchasi" dropdown ko'radi. Tanlangan tuman URL ga saqlanishi kerak: `?district=Bekobod%20tumani`. Default = "Barchasi". Bu link ulashishni ta'minlaydi.

3. **Sidebar**: `foydalanuvchilar` linki direktor uchun ko'rsatilmasin. Direktor `/api/users/*` endpointlariga 403 oladi.

4. **Removal workflow UI**: 
   - Tashkilot direktori: "Chiqarish taklif qilish" tugmasi (`propose-removal`)
   - Direktor: `chiqarilgan` sahifasida "Tasdiqlash kutilmoqda" tab → pending list (`/api/youth/removals`). Har bir row da sabab, kim taklif qilgani, sana va "Tasdiqlash" / "Rad etish" tugmalari.
   - Rad etishda izoh kiritish uchun dialog ochilsin (min 10 belgi).

5. **Override bilan biriktirish**: `assign-masul` da tuman farq qilsa, avval confirmation dialog ko'rsating: "Tuman farq qiladi, davom etasizmi?" → foydalanuvchi tasdiqlasa `?override=true` bilan so'rov yuboring.

6. **Stats va taqqoslash**: `/api/stats/compare?a=...&b=...` dan monitoring sahifasida 2 ta tuman tanlash orqali yonma-yon statistika ko'rsating. `from`/`to` date picker bilan birga ishlating.

7. **Drill-down**: Monitoring sahifasidagi tuman qatorini bosish → `yoshlar` sahifasiga `?district=...` filter bilan o'tkazsin.

8. **Export**: Dashboard va monitoring sahifasida "Hisobotni eksport qilish" tugmasi. v1 — CSV (`/api/reports/agency.csv?from&to`). PII kerak bo'lsa avval `POST /api/pii/reveal` chaqirilsin.

9. **Optimistic updates**: PATCH endpointlar yangilangan entity qaytaradi — cache ni shu bilan yangilang, keyin background refetch.

10. **Masul hodim cheklovi**: Masul faqat `contact` va `notes` ni o'zgartira oladi. UI da boshqa fieldlarni `disabled` qiling.
